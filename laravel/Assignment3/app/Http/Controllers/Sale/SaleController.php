...
class SaleController extends Controller
{
  ...
  /**
   * To search sale with all keywords
   * @return view sale list
   */
  public function searchSale()
  {
    ...
    if($search_text != "") {
      $saleList = Sale::where('customerID', 'LIKE', '%'.$search_text.'%')
      ->orWhere('productID', 'LIKE', '%'.$search_text.'%')
      ->orWhere('ordered_date', 'LIKE', '%'.$search_text.'%')
      ->get();
    }
    else {
      $saleList = sale::where('created_at', '>=', $from)
      ->where('created_at', '<=', $to)
      ->get();
    }
